...
final class Adjuster_Notices extends Adjuster {
	...
	public function apply_markup_adjustments( DOMDocument $doc, DOMXPath $xpath ): DOMDocument {
		...
		if ( false !== $notice_password_reset && ! empty( $notice_password_reset[0] ) ) {
			...
			/**
			 * Assign the raw text; DOMDocument escapes nodeValue on serialization,
			 * so escaping here would double-encode entities like "&".
			 */
			$notice_password_reset->nodeValue = $config['notice_password_reset'];
		}
		...
		if ( false !== $notice_register && ! empty( $notice_register[0] ) ) {
			 ...
			 * so escaping here would double-encode entities like "&".
			 */
			$notice_register->nodeValue = $config['notice_register'];
		}
		...
		if ( true === $config['show_custom_notice'] && ! empty( $config['notice_custom'] ) ) {
			...
			if ( false !== $login_form && ! empty( $login_form[0] ) ) {
				...
				/**
				 * Convey the notice severity without relying on color alone: an ARIA
				 * role plus a screen-reader-only prefix.
				 */
				$is_error = 'error' === $config['custom_notice_type'];

				if ( 'success' === $config['custom_notice_type'] ) {
					$severity_label = __( 'Success:', 'bm-custom-login' );
				} elseif ( $is_error ) {
					$severity_label = __( 'Error:', 'bm-custom-login' );
				} else {
					$severity_label = __( 'Notice:', 'bm-custom-login' );
				}

				/**
				 * Create a new div element for the custom notice. The text is added
				 * via a text node so DOMDocument escapes it exactly once.
				 */
				$custom_notice = $doc->createElement( 'div' );

				$custom_notice->setAttribute( 'class', implode( ' ', $class_names ) );
				$custom_notice->setAttribute( 'role', $is_error ? 'alert' : 'status' );

				$severity_prefix = $doc->createElement( 'span' );

				$severity_prefix->setAttribute( 'class', 'screen-reader-text' );
				$severity_prefix->appendChild( $doc->createTextNode( $severity_label ) );

				$custom_notice->appendChild( $severity_prefix );
				$custom_notice->appendChild( $doc->createTextNode( ' ' . $config['notice_custom'] ) );
