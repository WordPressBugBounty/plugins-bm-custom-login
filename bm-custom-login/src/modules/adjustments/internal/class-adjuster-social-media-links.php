	...
	public function apply_markup_adjustments( DOMDocument $doc, DOMXPath $xpath ): DOMDocument {
		...
		if ( true === $config['show'] && ! empty( $config['links_list'] ) ) {
			...
			if ( false !== $sibling && ! empty( $sibling[0] ) ) {
				...
				/**
				 * Create the wrapper container as a labelled navigation landmark
				 * so assistive tech can perceive and skip the group of links.
				 */
				$wrapper = $doc->createElement( 'nav' );

				$wrapper->setAttribute( 'class', 'bm-custom-login__social-media-links-wrapper' );
				$wrapper->setAttribute( 'aria-label', __( 'Social media', 'bm-custom-login' ) );

				// For each link, create and inject the markup.
				foreach ( $config['links_list'] as $link_data ) {
					/**
					 * Each link is icon-only, so it needs an accessible name. Fall
					 * back to a name derived from the icon when none was provided,
					 * so the link is never left unlabelled.
					 */
					$aria_label = trim( Utils\Type::ensure_string( $link_data['aria_label'] ) );

					if ( '' === $aria_label ) {
						$aria_label = ucwords( str_replace( [ 'fontawesome-', '-' ], [ '', ' ' ], $link_data['icon'] ) );
					}
					...
					// Raw value: DOMDocument escapes it once on serialization.
					...
					if ( ! empty( $link_data['icon'] ) ) {
						...
						/**
						 * Hide the decorative icon from assistive tech; the link's
						 * aria-label already conveys its purpose.
						 */
						if ( $fragment->firstChild instanceof DOMElement ) {
							$fragment->firstChild->setAttribute( 'aria-hidden', 'true' );
							$fragment->firstChild->setAttribute( 'focusable', 'false' );
						}
