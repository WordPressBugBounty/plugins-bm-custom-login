...
namespace Teydea_Studio\Custom_Login\Modules\Settings_Adopter;
...
final class Module_Settings_Adopter extends Utils\Module {
	...
	public function maybe_adapt_settings( array $data ): array {
		...
		/**
		 * Normalize the legacy option so every consumed key is present; a partially
		 * written legacy row would otherwise emit "undefined array key" warnings below.
		 */
	 ...
	 *
	 * @return string Hex colour, or an empty string if the result is invalid.
	 */
	private function adjust_brightness( string $hex, int $steps ): string {
		...
		return Utils\Type::ensure_string( sanitize_hex_color( $return ) );
	}
}
