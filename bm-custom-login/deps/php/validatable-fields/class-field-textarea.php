 ...
 * Field_Textarea class
 *
 * Multi-line counterpart to {@see Field_String}. Identical shape and storage
 * type, but the default sanitization pipeline uses `sanitize_textarea_field()`
 * instead of `sanitize_text_field()` so `\n` / `\r` survive — required for
 * email bodies, restricted-phrases lists, and anything else that the admin
 * edits in a `<textarea>`.
 */
final class Field_Textarea extends Field {
	/**
	...
	protected function sanitize_value( $value ): string {
		...
		if ( false === $this->skip_default_sanitization ) {
			/**
			 * Plain `trim()` rather than `Utils\Strings::trim()` — the latter
			 * collapses every run of whitespace (including `\n`) to a single
			 * space, which would defeat the whole point of this field type.
			 */
